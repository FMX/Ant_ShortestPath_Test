	...
	public static void main(String[] args) {
		...
		if (InOut.dijkstra_flag) {
			...
			DijkstraShortestPath<Integer, DefaultWeightedEdge> dijkstraShortestPath = new DijkstraShortestPath<Integer, DefaultWeightedEdge>(
					InOut.problem.graph, InOut.problem.start_vertex,
					InOut.problem.end_vertex);
			InOut.problem.shortest_path_edge_list = dijkstraShortestPath
					.getPathEdgeList();
			InOut.problem.shortest_path_length = dijkstraShortestPath
					.getPathLength();
		} else {
			System.out.println("using ACO algorithm");
			Ants.pheromone = Utilities.generate_double_matrix(
					InOut.problem.graph.vertexSet().size(), InOut.problem.graph
							.vertexSet().size());
			Ants.total = Utilities
					.generate_double_matrix(InOut.problem.graph.vertexSet()
							.size(), InOut.problem.graph.vertexSet().size());
		...
		for (DefaultWeightedEdge edge : InOut.problem.graph.edgeSet()) {
			System.out.println(edge + "--"
					+ InOut.problem.graph.getEdgeWeight(edge));
		}
		System.out.println("shortest_path_length: "
				+ InOut.problem.shortest_path_length);
		System.out.println("shortest_path_edge_list: "
				+ InOut.problem.shortest_path_edge_list);
