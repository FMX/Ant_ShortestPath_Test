...
public class Parse {
	...
	public static int parse_commandline(String args[]) {
		...
		options.addOption("i", "Graph file", true,
				"Graph inputfile (given format necessary)");
		options.addOption("o", "optimum", true,
				"# stop if tour better or equal optimum is found");
		options.addOption("m", "ants", true, "# number of ants");
		// options.addOption("g", "nnants", true,
		// "# nearest neighbours in tour construction");
		...
		options.addOption("e", "rho", true,
				"# rho: pheromone trail evaporation");
		options.addOption("q", "q0", true,
				"# q_0: prob. of best choice in tour construction");
		options.addOption("c", "elitistants", true, "# number of elitist ants");
		options.addOption("f", "rasranks", true,
				"# number of ranks in rank-based Ant System");
		// options.addOption("k", "nnls", true,
		// "# No. of nearest neighbors for local search");
		// options.addOption("l", "localsearch", true,
		// "0:no local search  1:2-opt  2:2.5-opt  3:3-opt");
		// options.addOption("d", "dlb", false,
		// "1 use don't look bits in local search");
		options.addOption("u", "as", false, "apply basic Ant System");
		options.addOption("v", "eas", false, "apply elitist Ant System");
		options.addOption("w", "ras", false,
				"apply rank-based version of Ant System");
		// options.addOption("x", "mmas", false,
		// "apply MAX-MIN ant_colony system");
		options.addOption("y", "bwas", false,
				"apply best-worst ant_colony system");
		...
		options.addOption("quiet", false,
				"reduce output to a minimum, no extra files");
		...
		if (cmd.hasOption("h")) {
			...
			formatter.printHelp("ACO algorithms for the Shortest Path"
					+ " [OPTION]... [ARGUMENT]...", "Options:", options, "");
			System.exit(0);
		}
		...
		if (cmd.hasOption("t")) {
			...
			System.out.println("-t/time Time limit with argument "
					+ InOut.max_time);
		} else {
			System.out.println("Note: Time limit is set to default "
					+ InOut.max_time + " seconds");
		}

		if (cmd.hasOption("r")) {
			InOut.max_tries = Integer.valueOf(cmd.getOptionValue("r"));
			System.out.println("-r/tries Number of tries with argument "
					+ InOut.max_tries);
		} else {
			System.out.println("Note: Number of tries is set to default "
					+ InOut.max_tries);
		}

		if (cmd.hasOption("s")) {
			InOut.max_solutions = Integer.parseInt(cmd.getOptionValue("s"));
			System.out.println("-s/tours Maximum number tours with argument "
					+ InOut.max_solutions);
		} else {
			System.out.println("Note: Maximum number tours is set to default "
					+ InOut.max_solutions);
		}
		...
		if (cmd.hasOption("seed")) {
			...
			System.out.println("Note: A seed was generated as "
					+ Utilities.seed);
		}

		if (cmd.hasOption("o")) {
			InOut.optimal = Double.valueOf(cmd.getOptionValue("o"));
			System.out.println("-o/optimum Optimal solution with argument "
					+ InOut.optimal);
		} else {
			System.out
					.println("Note: Optimal solution value is set to default "
							+ InOut.optimal);
		}
		...
		if (algorithmCount > 1) {
			System.err
					.println("Error: More than one ACO algorithm enabled in the command line.");
			System.exit(1);
		...
		/*
		 * Local search if (cmd.hasOption("l")) { LocalSearch.ls_flag =
		 * Integer.parseInt(cmd.getOptionValue("l"));
		 * 
		 * switch (LocalSearch.ls_flag) { case 0: System.out.println(
		 * "Note: local search flag is set to default 0 (disabled)"); break;
		 * case 1:
		 * System.out.println("Note: local search flag is set to default 1 (2-opt)"
		 * ); break; case 2: System.out.println(
		 * "Note: local search flag is set to default 2 (2.5-opt)"); break; case
		 * 3:
		 * System.out.println("Note: local search flag is set to default 3 (3-opt)"
		 * ); break; default: System.out.println("-l/localsearch with argument "
		 * + LocalSearch.ls_flag); break; } } if (LocalSearch.ls_flag != 0) {
		 * InOut.set_default_ls_parameters(); }
		 */

		if (cmd.hasOption("m")) {
			Ants.n_ants = Integer.valueOf(cmd.getOptionValue("m"));
			System.out.println("-m/ants Number of ants with argument "
					+ Ants.n_ants);
		} else {
			System.out.println("Note: Number of ants is set to default "
					+ Ants.n_ants);
		}
		...
		if (cmd.hasOption("c")) {
			...
			System.out
					.println("-c/elitistants Number of elitist ants with argument "
							+ Ants.elitist_ants);
		} else {
			System.out
					.println("Note: Number of elitist ants is set to default "
							+ Ants.elitist_ants);
		}

		if (cmd.hasOption("f")) {
			Ants.ras_ranks = Integer.valueOf(cmd.getOptionValue("f"));
			System.out.println("-f/rasranks Number of ranks with argument "
					+ Ants.ras_ranks);
		} else {
			System.out.println("Note: Number of ranks is set to default "
					+ Ants.ras_ranks);
		}

		/*
		 * if (cmd.hasOption("k")) { LocalSearch.nn_ls =
		 * Integer.parseInt(cmd.getOptionValue("k"));
		 * System.out.println("-k/nnls Number nearest neighbours with argument "
		 * + LocalSearch.nn_ls); } else { System.out .println(
		 * "Note: Number nearest neighbours in local search is set to default "
		 * + LocalSearch.nn_ls); }
		 */

		/*
		 * if (cmd.hasOption("d")) { LocalSearch.dlb_flag = true;
		 * System.out.println("-d/dlb Don't-look-bits flag with argument " +
		 * LocalSearch.dlb_flag); } else {
		 * System.out.println("Note: Don't-look-bits flag is set to default " +
		 * LocalSearch.dlb_flag); }
		 */
