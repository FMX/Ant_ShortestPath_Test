...
public class InOut {
	 ...
	 * ################################################
	 * 
	 * Version: 1.0 File: InOut.c Author: Thomas Stuetzle Purpose: mainly input
	 * / output / statistic routines Check: README and gpl.txt Copyright (C)
	 * 2002 Thomas Stuetzle
	 */
	...
	static int restart_iteration; /*
								 * remember iteration when restart was done if
								 * any
								 */
	...
	static double branching_factor; /*
									 * average node branching factor when
									 * searching
									 */
	static double found_branching; /*
									 * branching factor when best solution is
									 * found
									 */

	static int found_best; /* iteration in which best solution is found */
	static int restart_found_best;/*
								 * iteration in which restart-best solution is
								 * found
								 */
	...
	/*
	 * FUNCTION: parse and read Tsp.instance file INPUT: Tsp.instance name
	 * OUTPUT: list of coordinates for all nodes COMMENTS: Instance files have
	 * to be in TSPLIB format, otherwise procedure fails
	 */
	{
		String buf;
		int i;
		...
		Reader reader = new InputStreamReader(
				new FileInputStream(tsp_file_name), "UTF8");
		...
		while (line != null) {
			...
			if (!found_coord_section) {
				if (line.startsWith("NAME")) {
					...
					System.err
							.println("Not a TSP Tsp.instance in TSPLIB format !!");
					System.exit(1);
					...
					if (buf.equals("EUC_2D")) {
						...
						System.err.println("EDGE_WEIGHT_TYPE " + buf
								+ " not implemented");
						System.exit(1);
					}
				}
		...
		if (!found_coord_section) {
			System.err
					.println("Some error ocurred finding start of coordinates from tsp file !!");
			System.exit(1);
		}
	 ...
	 * time) INPUT: none OUTPUT: none COMMENTS: none
	 */
	{
		System.out.println("best " + Ants.best_so_far_ant.tour_length
				+ ", iteration: " + iteration + ", time "
				+ Timer.elapsed_time());
		if (comp_report != null)
			printToFile(comp_report, "best " + Ants.best_so_far_ant.tour_length
					+ "\t iteration " + iteration + "\t tours " + n_tours
					+ "\t time " + time_used);
	}
	...
	/*
	 * FUNCTION: output default parameter settings INPUT: none OUTPUT: none
	 * COMMENTS: none
	 */
	{
	...
	static void set_default_ls_parameters() {
		...
		LocalSearch.nn_ls = 20; /*
								 * use fixed radius search in the 20 nearest
								 * neighbours
								 */
	...
	/*
	 * FUNCTION: set default parameter settings INPUT: none OUTPUT: none
	 * COMMENTS: none
	 */
	{
	 ...
	 * standard deviations, average distance, branching-factor and output to a
	 * file gathering statistics INPUT: none OUTPUT: none (SIDE)EFFECTS: none
	 */
	{
		int j, k;
		int[] l;
		...
		for (k = 0; k < Ants.n_ants - 1; k++)
			for (j = k + 1; j < Ants.n_ants; j++) {
				avg_distance += (double) Ants.distance_between_ants(
						Ants.ant[k], Ants.ant[j]);
			}
		...
		if (stat_report != null) {
			printToFile(stat_report, iteration + "\t" + pop_mean + "\t"
					...
					+ branching_factor + "\t"
					+ ((branching_factor - 1.) * (double) Tsp.n) + "\t"
	...
	/*
	 * FUNCTION: compute the average node lambda-branching factor INPUT: lambda
	 * value OUTPUT: average node branching factor (SIDE)EFFECTS: none COMMENTS:
	 * see the ACO book for a definition of the average node lambda-branching
	 * factor
	 */
	{
		int i, m;
	...
	/*
	 * FUNCTION: output a solution together with node coordinates INPUT: none
	 * OUTPUT: none COMMENTS: not used in the default implementation but may be
	 * useful anyway
	 */
	{
	 ...
	 * INPUT: trial number OUTPUT: none COMMENTS:
	 */
	{
		...
		if (report != null)
			printToFile(report, "Best: " + Ants.best_so_far_ant.tour_length
					+ "\t Iterations: " + found_best + "\t B-Fac "
					+ found_branching + "\t Time " + time_used + "\t Tot.time "
					+ Timer.elapsed_time());
	 ...
	 * finishes INPUT: none OUTPUT: none COMMENTS:
	 */
	{
		...
		if (report != null) {
			...
			printToFile(report, "Best try: " + best_tour_length
					+ "\t\t Worst try: " + worst_tour_length);
	...
	/*
	 * FUNCTION: initialize the program, INPUT: program arguments, needed for
	 * parsing commandline OUTPUT: none COMMENTS:
	 */
	{
	...
	/*
	 * FUNCTION: print distance matrix INPUT: none OUTPUT: none
	 */
	{
		int i, j;
	...
	/*
	 * FUNCTION: print heuristic information INPUT: none OUTPUT: none
	 */
	{
		int i, j;
	...
	/*
	 * FUNCTION: print Ants.pheromone trail values INPUT: none OUTPUT: none
	 */
	{
		int i, j;

		System.out.println("Ants.pheromone Trail matrix, iteration: "
				+ iteration + "\n");
		for (i = 0; i < Tsp.n; i++) {
			System.out.println("From " + i + ": ");
			for (j = 0; j < Tsp.n; j++) {
	 ...
	 * INPUT: none OUTPUT: none COMMENTS: this computation assumes that no
	...
	/*
	 * FUNCTION: print the tour *t INPUT: pointer to a tour OUTPUT: none
	 */
	{
		int i;
	...
	/*
	 * FUNCTION: print the tour *t to cmp.tsplibfile INPUT: pointer to a tour
	 * OUTPUT: none
	 */
	{
		int i;
	...
	/*
	 * FUNCTION: make a simple check whether tour *t can be feasible INPUT:
	 * pointer to a tour OUTPUT: none
	 */
	{
		int i, sum = 0;
	 ...
	 * report files INPUT: none OUTPUT: none
	 */
	{
