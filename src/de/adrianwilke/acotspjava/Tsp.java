...
public class Tsp {
	 ...
	 * ################################################
	 * 
	 * Version: 1.0 File: TSP.c Author: Thomas Stuetzle Purpose: TSP related
	 * procedures, distance computation, neighbour lists Check: README and
	 * gpl.txt Copyright (C) 2002 Thomas Stuetzle
	 */
	...
	static class problem {
		...
		int[][] nn_list; /*
						 * nearest neighbor list; contains for each node i a
						 * sorted list of n_near nearest neighbors
						 */
	}

	public static int n; /* number of cities in the instance to be solved */
	 ...
	 * computing distances for TSPLIB instances INPUT: two node indices OUTPUT:
	 ...
	 * integer for TSPLIB instances INPUT: two node indices OUTPUT: distance
	 * between the two nodes COMMENTS: for the definition of how to compute this
	 * distance see TSPLIB
	 */
	{
	 ...
	 * integer for TSPLIB instances INPUT: two node indices OUTPUT: distance
	 * between the two nodes COMMENTS: adapted from concorde code for the
	 * definition of how to compute this distance see TSPLIB
	 */
	{
		double deg, min;
		...
		dd = (int) (6378.388 * Math.acos(0.5 * ((1.0 + q1) * q2 - (1.0 - q1)
				* q3)) + 1.0);
		return dd;
	...
	/*
	 * FUNCTION: compute ATT distance between two nodes rounded to next integer
	 * for TSPLIB instances INPUT: two node indices OUTPUT: distance between the
	 * two nodes COMMENTS: for the definition of how to compute this distance
	 * see TSPLIB
	 */
	{
		double xd = instance.nodeptr[i].x - instance.nodeptr[j].x;
	...
	/*
	 * FUNCTION: computes the matrix of all intercity distances INPUT: none
	 ...
	 * INPUT: none OUTPUT: pointer to the nearest neighbor lists
	 */
	{
		int i, node, nn;
	...
	/*
	 * FUNCTION: compute the tour length of tour t INPUT: pointer to tour t
	...
	static boolean tsp_check_tour(int[] t) {
		...
		for (i = 0; i < size; i++) {
			if (used[t[i]] != 0) {
				System.err
						.println("error: solution vector has two times the value "
								+ t[i] + "(last position: " + i + ")");
				error = true;
			} else
				used[t[i]] = 1; // was true
		...
		if (!error)
			for (i = 0; i < size; i++) {
				if (used[i] == 0) {
					System.out.println("error: vector position " + i
							+ " not occupied");
					error = true;
				}
			}
